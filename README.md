...
Abacus is described in Garrison et al. (2019, MNRAS, 485, 3370),
where we detail its performance on the Schneider et al. (2016) code
comparison simulation, and in Garrison et al. (2018, ApJS, 236,
43), which released an early suite of 125 simulations from 40
...
Use of AbacusSummit should cite Maksimova et al. (2020) for the
...
Most of the simulations in AbacusSummit will be 6912^3 = 330 billion 
...
We use global time steps that begin capped at d(ln a)=0.025 but
...
into the data products.  Abacus uses a cubic grid of size CPD^3,
...
L0 halos can span up to 16 slabs (though most are much smaller, as
...
simple numpy arrays to the user.
...
that the particles are output in cell order.  We use zstd as the
...
Linux cksum utility.  We note that there is an AVX implementation
of cksum that offers more than a 10x speed up
