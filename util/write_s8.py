...
for line in fileinput.FileInput(table,inplace=1):
    if root in line:
        # if there are no neutrinos, s8_cb and s8_m are the same
        try:
            line = line.replace(line,line[:-1]+s8_m+' |'+s8_cb+'  | '+'\n')
        except:
            line = line.replace(line,line[:-1]+s8_m+' |'+s8_m+'  | '+'\n')
    print(line,end='')

# rename the power spectrum, ini-file and transfer function
os.rename(os.path.join(root,root+'.z2_tk.dat'),os.path.join(root,TRANSFER))
os.rename(os.path.join(root,root+'.ini'),os.path.join(root,INIFILE))
